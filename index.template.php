...
<body id="', $context['browser_body_id'], '" class="surface action_', !empty($context['current_action']) ? $context['current_action'] : (!empty($context['current_board']) ?
...
function template_body_above()
{
	...
	/* @to-do till we find better place for this fellas
...
function template_body_below()
{
	...
	echo '
			<a href="', $scripturl, '?action=credits"><span>SMF 2.1 RC2</span></a>
			<a href="https://www.simplemachines.org/"><span>Simple Machines</span></a>
			<a href="', $scripturl, '?action=help"><span>', $txt['help'], '</span></a>
			', (!empty($modSettings['requireAgreement'])) ? '<a href="' . $scripturl . '?action=help;sa=rules"><span>' . $txt['terms_and_rules'] . '</span></a>' : '', '
			<a href="#top_section"><span>', $txt['go_up'], ' &#9650;</span></a>';
	...
	if ($context['user']['is_logged'])
	{
		...
		if ($context['allow_pm'])
			echo '
				<li>
					<a href="', $scripturl, '?action=pm"', !empty($context['self_pm']) ? ' class="active"' : '', ' id="pm_menu_top"><img src="', $settings['images_url'] ,'/inbox.png" alt="inbox">', !empty($context['user']['unread_messages']) ? ' <span class="amt">' . $context['user']['unread_messages'] . '</span>' : '', '</a>
		...
		echo '
				<li>
					<a href="', $scripturl, '?action=profile;area=showalerts;u=', $context['user']['id'], '"', !empty($context['self_alerts']) ? ' class="active"' : '', ' id="alerts_menu_top"><img src="', $settings['images_url'],'/alerts.png" alt="alerts">', !empty($context['user']['alerts']) ? ' <span class="amt">' . $context['user']['alerts'] . '</span>' : '', '</a>
...
function theme_linktree($force_show = false)
{
	...
	{
		$pad_item = $link_num * 16;
		echo '
					<ul style="margin-left: ' . $pad_item .'px">
		...
		echo '
						<li class="button_', $act, '', !empty($button['sub_buttons']) ? ' subsections"' : '"', '>
							<a', $button['active_button'] ? ' class="active"' : '', ' href="', $button['href'], '"', isset($button['target']) ? ' target="' . $button['target'] . '"' : '', '>
								<span class="textmenu">', $button['title'], !empty($button['amt']) ? ' <span class="amt">' . $button['amt'] . '</span>' : '', '</span>
							</a>';
